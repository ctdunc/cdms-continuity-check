...
if do_test_all:
    file_all = file_all+"_all.txt"
    file_failed = file_failed + "_all.txt"
else:
    file_all = file_all+".txt"
    file_failed = file_failed + ".txt"
...
for i in range(len(expected)):
    ...
    do_test = False
    if not do_test_all and channels_of_interest and  combinations_of_interest:
        for chan in channels_of_interest:
            for comb in combinations_of_interest:
                found_line1 = False
                if "SQ" in comb[0] or "TES" in  comb[0]:
                    line1 = comb[0]+'_'+str(chan)
                    if  line1==row[0] or line1==row[1]:
                        found_line1 = True
                else:
                    line1 = comb[0]
                    if line1 in row[0] or line1 in row[1]:
                        found_line1 = True
                
                found_line2 = False
                if "SQ" in comb[1] or "TES" in comb[1]:
                    line2 = comb[1]+'_'+str(chan)
                    if  line2==row[0] or line2==row[1]:
                        found_line2 = True
                else:
                    line2 = comb[1]
                    if line2 in row[0] or line2 in row[1]:
                        found_line2 = True
                      

                if found_line1 and found_line2:
                    do_test = True
                    break
            if do_test:
                break  
    
    if not do_test_all and not do_test:
        continue
...
result_df = pd.read_csv(file_all, sep='\s+', header=0, skiprows = 2)

failed_df = result_df[result_df['Pass?'] != 'PASS']
f_failed = open(file_failed, "w+")
